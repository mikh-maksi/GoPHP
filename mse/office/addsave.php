...
/*
		...
		for ($i=0;$i<$number;$i++)
		{
			...
			$name_i = mysql_result($result,$i,'login');
			//$sp_name_id = mysql_result($result,$i,'id');
			if ($sp==$name_i) 
				{$cond = 1;
				$sp_name_id = mysql_result($result,$i,'id');
				//echo $sp_name_id."!!!!!!!!!!!!!!";
		 ...
		 if ($cond)
			...
			{
			//Проверка на существование логина спонсора
			$welldone = 1;	$errorsp = 'Пользователя с таким логином - не существует<br>Проверьте правильность ввода';
...
//		if($_POST['lasname_field'] == '') 					 {$welldone = 1;	$errors [3] = 'Введите Вашу Фамилию';}
//		if($_POST['firsname_field'] == '') 					 {$welldone = 1;	$errors [4] = 'Введите Ваше Имя'; }
